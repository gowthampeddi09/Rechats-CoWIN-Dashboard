...
class CowinDashboard extends Component {
  ...
  getCovidVaccinationData = async () => {
    ...
    if (response.ok) {
      const covidVaccinationData = await response.json()
      const updatedData = {
        last7DaysVaccination: covidVaccinationData.last_7_days_vaccination.map(
          eachDay => ({
        ...
        vaccinationByAge: covidVaccinationData.vaccination_by_age.map(
          ageRange => ({
            age: ageRange.age,
            count: ageRange.count,
          }),
        ),
        vaccinationByGender: covidVaccinationData.vaccination_by_gender.map(
          genderType => ({
